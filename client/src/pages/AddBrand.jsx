...
import { brandSchema, editBrandSchema } from "../utils/schema";
...
const AddBrand = () => {
  ...
  const { addBrand, getBrand, editBrand, removeBrandImage } = useAppStore();
  ...
  const {
    ...
    onSubmit: async (values, action) => {
      if (id) {
        editBrand(values, navigate);
      } else {
        addBrand(values, navigate);
      }
    },
  });
  ...
  const removeImage = () => {
    if (id && values?.image?.public_id) {
      removeBrandImage(values, setValues);
    } else setValues((pre) => ({ ...pre, image: "" }));
  };

  useEffect(() => {
    if (id) {
      getBrand(id, setValues, navigate);
    }
  }, [id]);
